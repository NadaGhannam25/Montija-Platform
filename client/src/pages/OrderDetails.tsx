  ...
  if (!order) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center">
        ...
        <Button asChild className="mt-4"><Link href="/orders">عودة لطلباتي</Link></Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 min-h-screen space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-semibold">تفاصيل الطلب #{order.id}</span>
      </div>
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="font-semibold">
                {order.createdAt ? format(new Date(order.createdAt), "dd MMMM yyyy", { locale: arSA }) : "—"}
              </p>
            </div>
          </div>
      ...
      <div className="bg-card border border-border/50 rounded-3xl p-6 shadow-sm">
          ...
          {order.items?.map(item => (
            <div key={item.id} className="flex items-center gap-4 p-3 rounded-2xl bg-muted/30 hover:bg-muted/50 transition-colors" data-testid={`row-order-item-${item.id}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-muted-foreground mt-0.5">الكمية: {item.quantity}</p>
              </div>
              <div className="text-start shrink-0">
                <p className="font-black text-primary">{(Number(item.price) * item.quantity).toFixed(2)} ر.س</p>
                <p className="text-xs text-muted-foreground">{Number(item.price).toFixed(2)} ر.س / الوحدة</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-6 pt-5 border-t border-border/50 space-y-3">
          ...
          <div className="flex justify-between text-muted-foreground text-sm">
            ...
            <span>{delivery.toFixed(2)} ر.س</span>
          </div>
          <div className="flex justify-between font-black text-xl pt-3 border-t border-border/50">
            <span>الإجمالي</span>
            <span className="text-primary" data-testid="text-order-total">{Number(order.totalAmount).toFixed(2)} ر.س</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="bg-card border border-border/50 rounded-3xl p-6 shadow-sm">
          ...
          <p className="font-semibold" data-testid="text-payment-method">{PAYMENT_LABELS[order.paymentMethod] || order.paymentMethod}</p>
          <p className="text-sm text-muted-foreground mt-1">
            {status === "تم التسليم" ? "تم الدفع بنجاح" : "سيتم الدفع عند الاستلام أو عند المعالجة"}
          </p>
        </div>
      </div>
      ...
      {order.deliveryPersonName && status === "خرج للتوصيل" && (
        <div className="bg-card border border-primary/20 rounded-3xl p-6 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-muted-foreground">المندوب المسؤول عن توصيل طلبك</p>
      ...
      <Button variant="outline" asChild className="w-full h-12 rounded-2xl text-base">
        <Link href="/orders">العودة لجميع طلباتي</Link>
      </Button>
    </div>
  );
}
